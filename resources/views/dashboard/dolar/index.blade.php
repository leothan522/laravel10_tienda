<li class="nav-item" xmlns:wire="http://www.w3.org/1999/xhtml">
    {{--@if(leerJson(Auth::user()->permisos, 'precio.dolar')|| Auth::user()->role == 1 || Auth::user()->role == 100)--}}
        <button class="nav-link btn btn-block" @if(comprobarPermisos('precio.dolar')) wire:click="edit('{{ $edit }}')" @endif>
...
@else
    <li class="nav-item text-center" xmlns:wire="http://www.w3.org/1999/xhtml">
        <form class="row justify-content-center" wire:submit.prevent="save">
            <div class="input-group col-md-6 input-group-sm">
                <input type="text" step=".01" class="form-control text-decoration-none" wire:model.defer="dollar" required>
